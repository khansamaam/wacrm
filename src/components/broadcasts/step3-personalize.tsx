...
interface Step3Props {
  ...
  onCarouselCardMediaUrlsChange: (urls: string[]) => void;
  onNext: () => void;
  onBack: () => void;
}
...
export function Step3Personalize({
  ...
  const [uploadingCarouselCard, setUploadingCarouselCard] = useState<
    number | null
  >(null);
  ...
  const placeholders = useMemo(() => {
    const texts = [
      template.body_text,
      ...(template.carousel_cards ?? []).map((card) => card.body_text),
    ];
    const matches = texts.flatMap((text) => text.match(/\{\{(\d+)\}\}/g) ?? []);
    return [...new Set(matches)].sort((a, b) => {
      const left = Number(a.replace(/\D/g, ''));
      const right = Number(b.replace(/\D/g, ''));
      return left - right;
    });
  }, [template.body_text, template.carousel_cards]);
  ...
  const carouselCards = useMemo(
    () =>
      (template.template_type ?? 'standard') === 'carousel'
        ...
        : [],
    [template.template_type, template.carousel_cards]
  );

  async function uploadCarouselImage(cardIndex: number, file: File) {
    if (!['image/jpeg', 'image/png'].includes(file.type)) {
      toast.error(t('personalize.imageUploadInvalid'));
      return;
    }
    if (file.size > MEDIA_MAX_BYTES_BY_KIND.image) {
      toast.error(t('personalize.imageUploadTooLarge'));
      return;
    }

    setUploadingCarouselCard(cardIndex);
    try {
      const { publicUrl } = await uploadAccountMedia('chat-media', file);
      const next = [...carouselCardMediaUrls];
      next[cardIndex] = publicUrl;
      onCarouselCardMediaUrlsChange(next);
      toast.success(t('personalize.imageUploadSuccess'));
    } catch (error) {
      toast.error(
        t('personalize.imageUploadFailed', {
          message: error instanceof Error ? error.message : 'Unknown error',
        })
      );
    } finally {
      setUploadingCarouselCard(null);
    }
  }
  ...
  const invalidCarouselCards = useMemo(
    () =>
      carouselCards.flatMap((_, index) =>
        isValidHttpUrl(carouselCardMediaUrls[index]?.trim() ?? '')
          ? []
          : [index + 1]
      ),
    [carouselCards, carouselCardMediaUrls]
  );
    ...
    <div className="space-y-6">
      ...
      {carouselCards.length > 0 && (
        <div className="border-border bg-card/50 space-y-3 rounded-xl border p-4">
          <div>
            <p className="text-foreground text-sm font-medium">
              {t('personalize.carouselMedia')}
            </p>
            <p className="text-muted-foreground mt-1 text-xs">
              {t('personalize.carouselMediaDesc')}
            </p>
          </div>

          {carouselCards.map((card, cardIndex) => {
            const mediaUrl = carouselCardMediaUrls[cardIndex] ?? '';
            const isUploading = uploadingCarouselCard === cardIndex;
            return (
              <div
                key={cardIndex}
              ...
              >
                <p className="text-foreground text-xs font-medium">
                  {t('personalize.carouselCard', { number: cardIndex + 1 })}
                </p>
                <Input
                  type="url"
                  value={mediaUrl}
                  onChange={(event) => {
                    const next = [...carouselCardMediaUrls];
                    next[cardIndex] = event.target.value;
                    onCarouselCardMediaUrlsChange(next);
                  }}
                  placeholder={t('personalize.imageUrlPlaceholder')}
                  className="border-border bg-muted text-foreground placeholder:text-muted-foreground"
                ...
                {card.header_type === 'image' && (
                  <label className="border-border text-foreground hover:bg-muted inline-flex h-9 cursor-pointer items-center gap-2 rounded-md border px-3 text-xs">
                    {isUploading ? (
                    ...
                    {isUploading
                      ? t('personalize.uploadingImage')
                      : t('personalize.uploadImage')}
                    <input
                      type="file"
                      accept="image/jpeg,image/png"
                      className="sr-only"
                      disabled={uploadingCarouselCard !== null}
                      onChange={(event) => {
                        const file = event.target.files?.[0];
                        if (file) void uploadCarouselImage(cardIndex, file);
                        event.target.value = '';
                      }}
                    />
                  </label>
                ...
                {card.header_type === 'image' &&
                  isValidHttpUrl(mediaUrl.trim()) && (
                    // eslint-disable-next-line @next/next/no-img-element
                    <img
                      src={mediaUrl.trim()}
                      alt={t('personalize.carouselCard', {
                        number: cardIndex + 1,
                      })}
                      className="border-border max-h-36 rounded-md border object-contain"
                    />
                  )}
              </div>
            );
          ...
          {invalidCarouselCards.length > 0 && (
            <p className="text-xs text-amber-300">
              {t('personalize.carouselMediaMissing', {
                cards: invalidCarouselCards.join(', '),
              })}
            </p>
          )}
        </div>
