  ...
  try {
    ...
    if (!code || !phoneNumberId || !wabaId) {
      return NextResponse.json(
        { error: 'Embedded Signup did not return code, phone_number_id and waba_id' },
        { status: 400 },
      )
    }
    if (label.length > 80) {
    ...
    if (!appId || !appSecret) {
      return NextResponse.json(
        { error: 'META_APP_ID and META_APP_SECRET are required on the server' },
        { status: 503 },
      )
    }
    ...
    try {
      ...
      return NextResponse.json(
        { error: error instanceof Error ? error.message : 'Embedded Signup exchange failed' },
        { status: 400 },
      )
    }
    ...
    try {
      ...
      return NextResponse.json(
        { error: error instanceof Error ? error.message : 'Meta asset verification failed' },
        { status: 400 },
      )
    }
    ...
    if (error) {
      ...
      return NextResponse.json({ error: 'Failed to save coexistence number' }, { status: 500 })
    }

    const number = data as unknown as { id: string }
    const { error: jobError } = await supabase.from('whatsapp_sync_jobs').insert([
      { whatsapp_number_id: number.id, account_id: accountId, sync_type: 'history' },
      { whatsapp_number_id: number.id, account_id: accountId, sync_type: 'contacts' },
    ])
    if (jobError) {
      console.warn('[embedded-signup/complete] sync job creation failed:', jobError.message)
    }
