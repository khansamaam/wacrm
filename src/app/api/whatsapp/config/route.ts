...
export async function POST(request: Request) {
  try {
    ...
    if (requestedAccountId && requestedAccountId !== currentAccountId) {
      return NextResponse.json(
        { error: 'You can configure only the active workspace' },
        { status: 403 }
      );
    }
...
export async function DELETE(request: Request) {
  try {
    // Client Admins own their workspace connection. A Platform Super Admin
    // receives the same role only after entering an audited workspace
    // context, so this endpoint never accepts an arbitrary tenant id.
    const ctx = await requireRole('owner');
    const requestedAccountId = new URL(request.url).searchParams.get(
      'accountId'
    );
    if (
      requestedAccountId?.trim() &&
      requestedAccountId.trim() !== ctx.accountId
    ) {
      return NextResponse.json(
        { error: 'You can reset only the active workspace' },
        { status: 403 }
      );
    }
